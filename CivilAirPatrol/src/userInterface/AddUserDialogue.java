...
public class AddUserDialogue {
	...
	public AddUserDialogue(boolean deletion) {
		...
		if (!deletion) {
	        ...
	        userPasswordField1.setDocument(new TextDocumentForLimitedTextFields(8,1));
	        ...
	        userPasswordField2.setDocument(new TextDocumentForLimitedTextFields(8,1));
        ...
        frame.setPreferredSize(new Dimension(USER_DIALOGUE_WIDTH, USER_DIALOGUE_HEIGHT));
        frame.pack();
    ...
    private class createUserListener implements ActionListener {
        public void actionPerformed(ActionEvent e) {
        	...
        	if (user != null) {
        		createWarningMessage("User with that name already exists! Aborting.");
        	} else if (userNameField.getText().length() == 0) {
        		createWarningMessage("User name cannot be empty!");
        	} else if (userPasswordField1.getText().equals(userPasswordField2.getText())) {
        		System.out.println(userNameField.getText().length());
        		//sqlServer.InsertUser(userNameField.getText(), userPasswordField1.getText(), "WRITER");
            	frame.dispose();
        	} else {
    ...
    public void createWarningMessage(String s) {
        ...
        f.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
        f.setTitle("Warning");
        f.setPreferredSize(new Dimension(USER_DIALOGUE_WIDTH, USER_DIALOGUE_HEIGHT));
        f.pack();
